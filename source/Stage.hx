				...
				default:
					{
						...
						var stageFront:FlxSprite = new FlxSprite(-200, 500).loadGraphic(Paths.image('stagefront'));
						...
						var woodbg:FlxSprite = new FlxSprite(-200, -500).loadGraphic(Paths.image('woodbg'));
					woodbg.setGraphicSize(Std.int(woodbg.width * 1));
					woodbg.updateHitbox();
					woodbg.antialiasing = FlxG.save.data.antialiasing;
					woodbg.scrollFactor.set(1, 1);
					woodbg.active = false;
					swagBacks['woodbg'] = woodbg;
					toAdd.push(woodbg);

					var boxes:FlxSprite = new FlxSprite(-575, -500).loadGraphic(Paths.image('boxes'));
					boxes.setGraphicSize(Std.int(boxes.width * 1));
					boxes.updateHitbox();
					boxes.antialiasing = FlxG.save.data.antialiasing;
					boxes.scrollFactor.set(1, 1);
					boxes.active = false;
					swagBacks['boxes'] = boxes;
					toAdd.push(boxes);
